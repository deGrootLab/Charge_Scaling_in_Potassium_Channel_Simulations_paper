g_submit -s md.tpr -deffnm md -multidir \
...
-days 2.1 -nslots 32  -ngpu 2 -addargs " -replex 5000 -hrex -plumed plumed.dat " -N A-pot \
-source "/home/chui/Software-2023-04-11/set_plumed2.8.2_gmx_2021.7.sh" -nolaunch

# sed the line with "#SBATCH -p"
sed -i 's/#SBATCH -p/#SBATCH -p p16                   # partitions to use, each partition has one GPU/g' jobscript.*
